...
#: The identity of the store this run produced. Written HERE, from the same export, because
#: the bundled store and every fixture generated beside it must be one artifact: a store
#: replaced out from under them — which `make ios-export` used to do, projecting the LIVE
#: 400-day gold over this committed offline 140-day one — otherwise failed ten Swift tests
#: with a 200-line lane-strip diff that read like a code defect rather than a stale fixture.
IDENTITY = _ROOT / "apps" / "ios" / "Tests" / "SwimZHKitTests" / "Fixtures" / "store_identity.json"
...
def _write_store() -> None:
    ...
    try:
        ...
        _write_identity(result)
        print(f"{STORE.relative_to(_ROOT)}: {result}")
    finally:
        gold.unlink(missing_ok=True)


def _write_identity(result: object) -> None:
    """Record which store the fixtures beside it describe.

    Not a staleness gate — it cannot go stale, because the one command that writes the store
    writes this in the same breath. It exists so the SWIFT side can tell the two failures
    apart: "these fixtures describe another store" is one sentence and one fix, where a
    per-field diff of the same mismatch is an hour of reading lane strips.
    """
    report = cast(Any, result).value
    IDENTITY.write_text(
        json.dumps(
            {
                "_note": (
                    "GENERATED by scripts/ios_fixtures.py beside the store it describes — do "
                    "NOT hand-edit. Regenerate with `make ios-fixtures`."
                ),
    ...
    print(f"{IDENTITY.relative_to(_ROOT)}: {report.content_hash[:12]}…")
