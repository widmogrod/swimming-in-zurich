...
table (→ the ``lockers`` AND ``rentals`` aspects via `providers.mietobjekt`; a page without
the table yields empty tuples, never a failure — a malformed price cell in a present table IS
one), and the pool's ``Admission``
...
def scrape_declared_sources(
    ...
    *,
    ...
    for source in declared_sources(catalog):
        ...
        # fail-fast, aborts the build like an unparseable timetable. Both halves of the one
        # table ride the extract: lockers (S1) and rentals (S2).
