...
_REGISTRY_JS = Path(__file__).resolve().parents[2] / "static" / "js" / "components" / "registry.ts"
...
def test_gallery_components_cross_check_the_js_registry() -> None:
    ...
    the two directly — the JS-side `registry.test.ts` guards the states per component;
    ...
    assert js_names, "failed to parse any REGISTRY keys from registry.ts"
...
def test_dev_serves_static_assets_no_store(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    app = create_app()
    with TestClient(app) as client:
        response = client.get("/static/js/app.ts")
    assert response.status_code == 200
