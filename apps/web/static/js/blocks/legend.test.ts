...
test('legendModel lists the 8 families, the 3 terminal states, and the elig key', () => {
  const m = legendModel();
  expect(m.families.length).toBe(8);
  expect(m.families.map((f) => f.family)).toEqual(['public', 'lane', 'family', 'women', 'seniors', 'adults', 'school', 'club']);
...
test('createBoardLegend renders swatches, the three states, the elig badges, and the note', () => {
  ...
  expect(famSwatches.length).toBe(8);
