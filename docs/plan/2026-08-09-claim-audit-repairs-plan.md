...
| 2026-08-09 | S4 | done | wire-level scope beyond the named Touches, forced by the acceptance: `apps/web/api/pools/{model,service}.py` — `address` becomes `str \| None` on the wire and both projections map the domain's empty-string sentinel to None (no JS consumer reads address — critic-verified); three comment-only touches outside Touches, each made false by the rule change (seed.py docstring + test_seed.py comment claimed Heuried's description IS "NULL"; wfs_snapshot caveat extended). Catalog re-snapshot byte-equal via MockTransport with generated_at preserved (critic re-executed); the roster golden upgraded id-set → full-entry equality; critic mutation check: neutralizing the sentinel rule fails 7 tests | uncleaned fields (tel/www/kategorie/poi_id) match the sentinel only as the exact raw token (" NULL " would survive; no committed input carries one); the whole-surface literal scan costs ~2s of suite time (accepted — honest scan); `name` deliberately exempt from the sentinel (identity-bearing, comment + corpus pin) | yes |
