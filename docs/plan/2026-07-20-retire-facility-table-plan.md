...
| 2026-07-20 | B2 | done | absorbed the read-flip-forced test migrations the plan nominally scoped to B4 (`test_sqlite_repo`, `test_gold_store_catalog_calendar`, `test_app_gold`, `test_cli`, plus `test_pipeline` — not on the B4 list but broken by the flip) — required to keep QA green per slice; critic-confirmed faithfully adapted, not weakened | **B2→B4 enrichment gap**: `scrape-gold`/`scrape-lanes` still write `write_gold`→the `facility` table (unread by the app), so scraped-only pools + City's scraped price/lane-plan are NOT on the `/swim` read path until B4 reroutes them (the 4 curated pools are unaffected — they serve via `write_schedules`); `PoolRow.facility_doc` is now an in-memory carrier only; single-writer of `facility_doc` is code-inspection-only until B5's guard | yes |
| 2026-07-20 | B3 | done | the single `is_curated` rule lives in `storage/codec.py`; removed `PoolRow.curation_status` field + `CURATED`/`UNCURATED` seed constants + build-time `_is_curated` (beyond a literal column drop) — a computed-but-unstored field would reintroduce the dual representation B3 exists to kill; deleted the now-obsolete `curation_status` CHECK-constraint test (feature gone — not a gate weakening; B5 adds the "no writable column" guard) | none — this consolidation removes prior dual-representation debt (retires #4 by construction). Discovery: once B4 reroutes scrapers through `write_schedules`, a scraped-only pool auto-flips to `curated` at read with no status writer to update (exactly B5's consistency-test scenario) | no |
| 2026-07-20 | B4 | done | **required a revision round** — the first cut left the `scrape-gold` reroute UNGUARDED (a `scrape_gold → write_gold` revert passed the whole suite); fixed with a scraped-only-pool read-path wiring test (`hallenbad-altstetten` can only reach `pool.facility_doc` via `write_schedules`), critic mutation-verified. City's scraped-price merge is proven via the internal `compose → write_schedules` seam over a price-stripped curated City (the live CLI can't show it — curated price wins + no fixture price) | **for Plan C:** `build_store` (`etl/build.py`) ALSO still calls `write_facilities`, so the `facility` table has **TWO** legacy writers (`pipeline.run` + `build_store`), not one as the B2/B4 prose says — Plan C must remove BOTH `write_facilities` sites + the geo-parity test's `SELECT doc FROM facility` | yes |
| 2026-07-20 | B5 | done | guards 1–3 (no runtime `FROM/JOIN facility` read; single writer of `pool.facility_doc`; no writable `curation_status` column) in `tests/test_facility_retirement_guards.py`; guard 4 (consistency: a scraped schedule flips the derived `curated` flag on the roster/`/pools` AND is served by `/swim`) in `apps/web/tests/api/test_curation_consistency.py`. Each critic-mutation-verified; tests-only, no source change | non-blocking (critic): the two line-based guards (facility read; `curation_status` write) would miss an SQL statement split across newlines — harmless today (all such SQL is single-line); guard 2 already matches whole-file text | no |
