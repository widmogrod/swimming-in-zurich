...
**Open:** the owner-label decision (above) · Pool-mode multi-basin rows and the phone-specific
treatment (research variant D) remain out of scope and now have real data behind them.

**Cleared 2026-08-11** (the ledger's recorded debt, all six items):
- Row identity is defined **once**, in the `blocks/rowkey.ts` leaf — `board.ts` and `poolrank.ts`
  both import downward, so the pure module never pulls in a DOM/canvas block.
- `poollist` no longer parses the label to decide what to render: `applyLabelRule` sets a
  `basinInLabel` flag. That removes the third row-label format definition and the last place a
  human-facing string decided behaviour (I6 in spirit).
- `gantt.ts` and `detailpanel.ts` call `isPublicSegment`; no `'PublicSwim'` literal remains in
  production outside `cursor.js`.
- Both surviving S4 mutants are pinned (a *named* public hold; `ownerLabelFits`' padding at the
  boundary), each verified to fail under its mutation.
- The three fixture generators are committed under `scripts/fixtures/`. The two frozen references
  reconstruct their pre-change world via `git archive` (S1 ← `8b7f954`, S2 ← `659c76a`) and refuse to
  overwrite without `--force`. Falsification that proves it: running S1's generator with
  `--commit HEAD` reports `32 -> 36` options, so it is genuinely replaying a pre-`_carry_bindings`
  world rather than re-deriving from today's code — the property AC4 depends on.
- `BoardRow.facility` stays required; `app.ts`'s `||` chain moved into the tested seam
  `board.ts::rowFacilityOf`, so the `''`-means-absent rule is pinned rather than implicit.

**Known limitation of that last fix:** the generators cannot be wired in as pytest guards —
`pyproject.toml` excludes `^scripts/` from mypy, so importing them would drag `scripts/` under strict
type-checking. They are verifiable by hand via `--check` only.

**Not introduced here, but found while clearing it:** `detailpanel.ts:117`'s `publicSpan` predicate is
unpinned — inverting it leaves the whole suite green.
